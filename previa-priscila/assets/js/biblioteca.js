/* ============================================================
   BIBLIOTECA DE MATERIAIS — área logada de Priscila Melo · Nutricionista.
...
(function () {
  ...
  function criarCard(c, eb, liberado) {
    ...
    var acao = liberado
      ...
      /* Sem o módulo de vendas não há como comprar aqui: quem quiser o
         material pede o acesso e a liberação é feita à mão no painel. */
      : (previa + '<button class="btn btn-outline btn-block bib-pedir" type="button">🔒 Pedir acesso</button>');
  ...
  function abrirLeitor(c, eb, btn) {
    ...
    c.storage.from("ebooks").download(eb.arquivo).then(function (res) {
      ...
      if (res.error || !res.data) {
        alert("Não foi possível abrir este material. Se você já tem acesso, fale com " + "Priscila" + ".");
