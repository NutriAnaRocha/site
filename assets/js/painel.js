/* ============================================================
   PAINEL — a página onde a Ana libera o acesso aos materiais.
...
(function () {
  ...
  window.SiteDBReady.then(function (c) {
    ...
    if (!r.data.session) { window.location.replace("entrar?next=painel"); return null; }
    return cliente.from("profiles").select("tipo").maybeSingle();
  }).then(function (r) {
    if (!r) return null;
    if (!r.data || r.data.tipo !== PAPEL_DONA) { window.location.replace("biblioteca"); return null; }
    return carregarTudo();
  }).catch(function (e) {
  ...
  document.addEventListener("click", function (e) {
    ...
    if (e.target.closest("[data-logout]")) {
      e.preventDefault();
      if (cliente) cliente.auth.signOut().then(function () { window.location.replace("/"); });
    }
